...
public class AdminService {
    ...
    public Map<String, Object> getDashboardStats() {
        ...
        try {
            // Thống kê users
            List<org.example.webtuthien.user.User> users = userService.getAllUsers();
            ...
            // Thống kê donations
            List<org.example.webtuthien.donation.Donation> donations = donationService.list();
            stats.put("totalDonations", donations.size());
            
            BigDecimal totalDonationAmount = donations.stream()
                .map(org.example.webtuthien.donation.Donation::getAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
